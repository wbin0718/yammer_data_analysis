## Yammer 데이터 분석 프로젝트
  ...
  * yammer가 제공하는 총 4개의 테이블을 활용하여 실제 한 기업의 데이터 분석가가 수행하는 단계인 **문제 정의**, **가설 정의**, **가설 검증**  절차를 진행 해 보는 프로젝트입니다.

## User Engagement 감소

<image src = "https://github.com/wbin0718/google_analytics_dashboard/assets/104637982/5d6d93ce-c6cc-4de0-8ccf-25de3f2a5872" width = 500>
 
### 문제 정의
 
* 위 그래프를 보면 7월 28일을 기점으로 WAU가 꾸준히 감소하고 있음.
* WAU가 감소하는 이유를 데이터를 통해 찾고자 함!

### 가설 설정

* 휴일이었나? -> 휴일이면 회사를 출근하지 않으니까 접속자 수가 적을 것!
* 신규 가입자가 적어졌을까? -> 신규 사용자의 감소로 인해 WAU가 감소하는 형태를 보일 수 있음!
* 특정 기능이 동작 오류가 발생했나? -> 특정 기능을 통해 사용자가 접속할 수 있는데, 이 기능이 제대로 동작되지 않아 사용자가 접속하지 못 한 것일 수도 있음!
* 로그 수집 도구 오류가 발생했나? -> 로그 수집 도구 문제로 인해 제대로 집계가 되지 못 한 것일 수도 있음!

### 가설 검증

#### 신규 사용자의 문제?

* 먼저 WAU가 감소한 이유를 신규 가입자의 감소로 부터 이어졌다는 가설을 확인!
...
* 일을 기준으로 전체 가입자와 신규 가입자의 수를 나타낸 그래프.    
* 그래프를 보면 정확히 신규 가입자가 감소했는지 판단하기는 어려운 것 같아 이전 주 대비 증가 및 감소 퍼센트 확인.

<image src = "https://github.com/wbin0718/yammer_data_analysis/assets/104637982/134d07ec-b527-47c6-a0b4-33ad5e13f77e" width = 500>

* 위 그래프를 보면 7월 28일 기준으로 신규 가입자 8월 4일 -19%가 감소를 했지만 8월 11일 주차부터는 다시 신규 가입자 상승.
* 따라서 신규 가입자가 어느 시점 감소는 있었지만 다시 증가하는 추세를 보이고 있어 꾸준히 감소하는 WAU의 원인은 아닌 것으로 판단!

#### 기존 사용자의 문제?

* 신규 가입자가 감소하는 것이 아니라면 기존 사용하고 있던 사용자의 문제로 WAU가 감소로 이어졌을 가능성 존재!

* 신규 가입자의 문제가 아니므로 어떤 사용자들이 어려움을 겪는지 확인하기 위해 가입 주차로 코호트를 나누어 리텐션 분석 진행.

<image src = "https://github.com/wbin0718/yammer_data_analysis/assets/104637982/bdf1699d-3451-47a1-8be1-21793af2fe72" width = 500>

* 가입 주차로 코호트를 나누고 9월 1일까지의 리텐션을 확인한 결과, 가입한지 10주 이상된 사용자들의 리텐션 비율이 8월 4일부터 급격히 떨어지고 있음.

#### 이메일의 문제?

* yammer는 가입한 고객들에게 일주일의 내용을 요약해서 이메일을 사용자에게 보내주고 사용자가 yammer를 접속하도록 유도하는 digest email 이벤트를 발생시킴.
* 신규 고객들은 다양한 마케팅 채널을 통해 접속하는 반면 기존 사용자는 마케팅 채널보다 이메일을 통해 접속하는 경우가 많다고 함!
* 그렇다면 장기 고객들이 주로 접속하는 이메일 채널에서 어떤 문제가 생긴 것은 아닐지 확인을 해 볼 필요가 있음!
...
* 위 그래프를 보면 이메일과 관련된 이벤트가 얼만큼 발생했는 지를 보여주고 있음.
* 고객들이 email을 open한 이벤트는 감소하지 않고 꾸준히 증가.
* 하지만 email을 클릭을 하고 이메일 안의 링크를 클릭하는 이벤트는 감소!

* 즉, 고객들이 이메일을 오픈은 하지만 이메일 안의 링크를 통해 yammer로 접속을 하지 않음.
* 조금 더 구체적으로 확인하기 위해 이메일을 오픈 클릭률과 이메일 내용 링크를 클릭한 클릭률을 확인 필요!
...
* 맨 아래 두 직선을 보면 이메일 오픈 클릭률은 증가하다가 살짝 감소 후 일정한 수준을 유지.
* 이메일을 오픈하고 내용 링크를 클릭한 비율은 증가하다가 꾸준히 감소.

### 분석 결과

* 분석한 것을 토대로 WAU가 감소한 원인은 기존 사용자 중 가입한 지 10주 이상 지난 사용자.
* 오래된 사용자들은 주로 이메일을 통해 접속.
* 이메일 오픈하는 클릭률은 상승하다가 일정 수준 유지. 하지만, 링크를 누르고 yammer 랜딩 페이지로 접속하는 클릭률은 조금씩 감소.
* 이 때, 사용자들이 이메일을 오픈은 잘 하지만 링크가 랜딩페이지로 연결하지 못하는 오류가 발생해서 WAU가 감소되는 것으로 추측해 볼 수 있음.
